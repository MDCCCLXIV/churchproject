    ...
    <div>
         ...
         <div className="block md:flex w-full h-full    md:h-full md:mx-auto md:max-w-6xl">
            <div className="flex w-full flex-col md:w-[35%] m-auto h-[30%]">
                ...
                <p className='text-base text-center'>We welcome you with great joy to Gethsemane life
                 Church. We are here to share the great news of Jesus Christ to all that will listen. 
                 Lorem ipsum dolor sit amet consectetur adipisicing elit. Officiis illo est nulla, beatae deserunt quis!</p>
            </div>
             <div className="flex flex-col w-full h-[70%] md:w-[60%] md:h-full p-2">
              ...
              <div className="grid w-full h-[100%] md:h-[70%]  md:m-auto gap-1 grid-cols-1 grid-flow-row md:grid-cols-2 md:grid-rows-2">
                  ...
                  {eventsData.map((event)=>{
                    return(
                    <div className='flex m-1 rounded-md shadow-xl bg-cover bg-center bg-no-repeat' style={{ backgroundImage: `url(${process.env.PUBLIC_URL}/${event.image})` }}>
                      <div className="flex bg-black/40 text-white w-full rounded">
                        <img className='object-center w-[100px] h-[100px] md:w-[120px] md:h-[120px] m-auto rounded-full object-cover'src={`${process.env.PUBLIC_URL}/${event.image}`}  alt="" />
                  
                  <div className="flex flex-col w-[60%]">
                    <div className="flex flex-col w-full h-[60%] m-auto">
                      <span className='w-full text-base md:text-lg text-center font-semibold flex m-auto'><LuCalendarDays className='mx-1 my-auto' size={20}/>{event.Date}</span>
